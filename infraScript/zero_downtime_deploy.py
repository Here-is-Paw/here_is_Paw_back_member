...
class ServiceManager:
    ...
    def __init__(self, socat_port: int = 8090, sleep_duration: int = 3) -> None:
        ...
        self.services: Dict[str, int] = {
            'here_is_paw_member_1': 8091,
            'here_is_paw_member_2': 8092
        }
    ...
    def _find_current_service(self) -> None:
        ...
        if not current_service:
            self.current_name, self.current_port = 'here_is_paw_member_2', self.services['here_is_paw_member_2']
        else:
    ...
    def _cleanup_dangling_images(self) -> None:
        ...
        # 이미지 정리
        cmd_remove = "docker images | grep 'here-is-paw/here_is_Paw_member' | grep '<none>' | awk '{print $3}' | xargs -r docker rmi"
    ...
    def _run_container(self, name: str, port: int) -> None:
        os.system(
            f"docker run -d --name={name} --restart unless-stopped -p {port}:8091 -e TZ=Asia/Seoul -v /dockerProjects/here_is_paw_msa/volumes/gen:/gen --pull always ghcr.io/here-is-paw/here_is_paw_member")
    ##
